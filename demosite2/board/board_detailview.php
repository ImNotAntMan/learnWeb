...
<body>
  ...
  if ($resultset->num_rows > 0) {
    ...
    while ($row = $resultset->fetch_assoc()) {
      echo "<tr><td>" . $row['userid'] . "</td><td>" . $row['username'] . "</td><td>" . $row['reply_subject'] . "</td><td width='50%'>" . $row['reply_contents'] . "</td><td>" . $row['reply_registdate'] ."</td><td><a href='board_deletereply.php?id=" . $row['replyid'] . "'>삭제</a></td></tr><br>";
    }
  }
    echo "</table><br>";
    echo "답글 리스트<br>";
    // $sql = "SELECT * FROM board, boardreply WHERE board.boardid = boardreply.boardid and board.boardid = ". $boardid." order by registdate desc";
    // $resultset = $conn->query($sql);
